...
object EntitySpawnModel {
  ...
  def handleSpawnEvent(entity:EntityType,player:SpaceCraftPlayer,num:Int = 1,postProcess:Entity => Unit = (_ => ())):SpaceCraftPlayer = {
    ...
    player.copy(
      postProcessing = () => {
        (0 to num).foreach(_ =>{
          val entity = player.value.getWorld.spawnEntity(
  ...
  case class DragonSpawnEvent(frequency:Double,probability:Double,value:Option[BukkitTask] = None) extends SpaceCraftPlayerEvent {
  ...
  case class BlazeSpawnEvent(frequency:Double,probability:Double,value:Option[BukkitTask] = None) extends SpaceCraftPlayerEvent {
    override def setFrequency(frequency: Double): SpaceCraftPlayerEvent = this.copy(frequency = frequency)
    override def setProbability(probability: Double): SpaceCraftPlayerEvent = this.copy(probability = probability)
  ...
  case class GhastSpawnEvent(frequency:Double,probability:Double,value:Option[BukkitTask] = None) extends SpaceCraftPlayerEvent {
    override def setFrequency(frequency: Double): SpaceCraftPlayerEvent = this.copy(frequency = frequency)
    override def setProbability(probability: Double): SpaceCraftPlayerEvent = this.copy(probability = probability)
  ...
  case class PhantomSpawnEvent(frequency:Double,probability:Double,value:Option[BukkitTask] = None) extends SpaceCraftPlayerEvent {
    override def setFrequency(frequency: Double): SpaceCraftPlayerEvent = this.copy(frequency = frequency)
    override def setProbability(probability: Double): SpaceCraftPlayerEvent = this.copy(probability = probability)
